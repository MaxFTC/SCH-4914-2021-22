...
class MyStateMachine {
    ...
    ElapsedTime timStepTimer = null;  				// Keeps elapsed time to operation within a single step.

    int iStep = 0; 									// tell us what step we are in.

    int iLastStep = -1; 							// Keep track of the last step executed. When iStep != iLastStep it means
    ...
    boolean bIsDone = false; 						// tell us if the state machine is complete. Public so that outside can tell when down.
    ...
    boolean bStepFirstPass = false;					// A new step is starting, se set this to false to allow is to set the step up.
    // in the first pass.
    ...
    public void setDelayStartTime( double dDelaySeconds ){

        this.dDelayStart = dDelaySeconds;
    ...
    public void updateSM(){							// update the State machine for a single pass.
        ...
        if( iStep == 0 && dDelayStart != 0.0){	// Sometimes we want to delay start our autonomuos because an alliance member
            ...
            if( timStepTimer.seconds() < dDelayStart  )
                return;
    ...
    public void auton0(){						// this will be over written in instance.
